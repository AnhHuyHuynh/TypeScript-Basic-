...
for (let i = 0; i < cacnutmuangay.length; i++) {
    ...
    cacnutmuangay[i].addEventListener("click", function () {
        ...
        if (tatcahang.getsanphamByID(idSanPham) === 0) {
            ...
            var sanphamlaydc = tatcahang.getsanphamByID(idSanPham);
            ...
            if (giohang.checksptruockhithem(sanphamlaydc) === -1) {
                ...
                var chuyendoisanphamsanggiohang = new SanPhamGioHang(sanphamlaydc, 1);
            ...
            for (var i = 0; i < soluongs.length; i++) {
                soluongs[i].addEventListener('change', function () {
                    ...
                    // lay id san pham
                    var idsanpham = parseInt(this.getAttribute("data-idsanpham"));
...
function XoaSanPhamGioHang() {
    ...
    for (var i = 0; i < btnXoaSp.length; i++) {
        btnXoaSp[i].addEventListener('click', function () {
            // lay id san pham
            var idsanpham = parseInt(this.getAttribute("data-idsanpham"));
            var sanphamlayduoc = tatcahang.getsanphamByID(idsanpham);
